...
def calculate_rotation_angle(joint1, joint2):
    ...
    #with open('/homes/omoussadek/CV/SLR_Sentiment_Enhanced/data-prepare/for_angle_analysis/bin10.txt', 'a') as file:
    #    print(f"Left_Shoulder_xyz: {joint1[0]}, {joint1[1]}, {joint1[2]}",file=file)
    #    print(f"Right_Shoulder_xyz: {joint2[0]}, {joint2[1]}, {joint2[2]}",file=file)
    #    print(f"delta_x:{delta_x}, delta_y={delta_y}, delta_z:{delta_z}",file=file)
...
for root, dirs, files in os.walk(npy_folder, topdown=False):
    for name in tqdm(files):
        ...
        bins_angle= np.zeros((36,3)) #ho 18 bin da 10 gradi ciascuno
        ...
        for k in range(npy.shape[0]):
            ...
            bin_no_depth = int(all_frames_angles[k][0] // 5)  #primo angolo
            bin_with_depth = int(all_frames_angles[k][1] // 5) #secondo angolo
            bin_for_correction = int(all_frames_angles[k][2] // 5) #terzo angolo
        ...
        for i in range(3):
            if 0 <= indices_max[i] <= 17:
                angle_discretized[i] = indices_max[i] * 5
            else:
                angle_discretized[i] = (36 - indices_max[i]) * -5
        ...
        #with open('/homes/omoussadek/CV/SLR_Sentiment_Enhanced/data-prepare/for_angle_analysis/bin30_sh.txt', 'a') as file:
        #    print(f"\n\n{name}\n", file=file)
        #    print(f"Right Shoulder coord: {right_shoulder[1][0]}, {right_shoulder[1][1]}, {right_shoulder[1][2]}", file=file)
        #    print(f"Left Shoulder coord: {left_shoulder[1][0]}, {left_shoulder[1][1]}, {left_shoulder[1][2]}",file=file)
        #    print(f"delta_x:{delta_x}, delta_y={delta_y}, delta_z:{delta_z}",file=file)
        #    print(f"angle discretized: {angle_discretized}",file=file)
        ...
        for i in range(npy.shape[0]):
            ...
            rotated3D_shoulders = rotate_joints_3d(npy[i, :, :3], angle_discretized[0]  ,angle_discretized[1], center3D) #ruoto anche rispetto alla profondità
